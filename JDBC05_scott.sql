...
SELECT * FROM TBL_EMP WHERE EMP_ID=1060
;

EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG

UPDATE TBL_EMP
SET EMP_ID='%d', EMP_NAME='%s', SSN='%s', IBSADATE='%s', CITY_ID='%d', TEL='%s', BUSEO_ID='%d', JIKWI_ID='%d', BASICPAY='%d', SUDANG='%d'
;

SELECT *
FROM TBL_EMP E JOIN TBL_CITY C JOIN TBL_BUSEO B
  E.BUSEO_ID = B.BUSEO_ID;
    , E.CITY_ID = C.CITY_ID;
...
SELECT EMP_ID , EMP_NAME, SSN, IBSADATE , CITY_LOC , TEL , BUSEO_NAME , JIKWI_NAME  , BASICPAY, SUDANG ,  NVL(BASICPAY*12,0)+NVL(SUDANG,0) AS PAY
...
--8. 직원 데이터 전체 출력(사번/이름/부서/직위/급여 내림차순)
...
WHERE EMP_ID ='이원영';
--==>>한 줄 구성
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE, CITY_LOC, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW WHERE EMP_ID ='이원영'
;

--10. 직원정보 수정
UPDATE TBL_EMP
SET EMP_NAME='이원일', SSN='801212-1234567', IBSADATE=TO_DATE('2012-11-11','YYYY-MM-DD'), CITY_ID=(SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC='제주'), TEL='010-9999-9999', BUSEO_ID=(SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME='총무부'), JIKWI_ID=(SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME='과장'), BASICPAY=500000, SUDANG=100000
WHERE EMP_ID=1069;
--> 한 줄 구성
UPDATE TBL_EMP SET EMP_NAME='이원일', SSN='801212-1234567', IBSADATE=TO_DATE('2012-11-11','YYYY-MM-DD'), CITY_ID=(SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC='제주'), TEL='010-9999-9999', BUSEO_ID=(SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME='총무부'), JIKWI_ID=(SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME='과장'), BASICPAY=500000, SUDANG=100000 WHERE EMP_ID=1069
;
...
SELECT E.EMP_ID, E.EMP_NAME, E.SSN, E.IBSADATE, C.CITY_LOC, E.TEL, B.BUSEO_NAME, J.JIKWI_NAME, J.MIN_BASICPAY, E.BASICPAY, E.SUDANG, (E.BASICPAY+E.SUDANG) AS PAY 
FROM TBL_EMP E, TBL_CITY C, TBL_BUSEO B, TBL_JIKWI J
WHERE E.CITY_ID = C.CITY_ID
  AND E.BUSEO_ID = B.BUSEO_ID
  AND E.JIKWI_ID = J.JIKWI_ID
ORDER BY E.EMP_ID ASC;  --사번 번호
...
1068	김가나	112233-4455667	2019-04-24	충북	010-1111-2222	총무부	대리	1250000	1260000	10000	1270000
1069	이원영	901212-1234567	2010-11-11	서울	010-1234-1234	개발부	대리	1250000	2160000	2000000	4160000
*/
